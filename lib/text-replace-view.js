         ...
         (beforeIndexAfter[i][2] == beforeIndexAfter[j][2])){
           ...
           if(iLevel == jLevel && isNumeric(beforeIndexAfter[i][1])){
             ...
             replacedText = replaceStrAtWithBIAUpdate(replacedText, parseFloat(beforeIndexAfter[j][3]) + 1,
                              updateValue,
                              beforeIndexAfter[j][1].length, beforeIndexAfter);
 ...
 */
function replaceStrAtWithBIAUpdate(orgStr, index, replaceWith, replaceLen, biaArray){
  replaceLen = replaceLen || replaceWith.length;
  ...
  for(var i = 0; i < biaArray.length; i++){
    if(biaArray[i][0] > index+replaceLen){
      // shift
      biaArray[i][0] = biaArray[i][0] + (replaceWith.length - replaceLen);
    }
    if(biaArray[i][0] > index+replaceLen){
      // shift
      biaArray[i][1] = biaArray[i][1] + (replaceWith.length - replaceLen);
    }
  }
