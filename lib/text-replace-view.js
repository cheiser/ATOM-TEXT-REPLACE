...
export default class TextReplaceView {
  ...
  constructor(serializedState) {
    ...
    fltArea.rows = 1;
    fltArea.cols = 100;
    fltArea.addEventListener("keyup", function(event){
                                        pressedKeyFilterEHandler(event, _this);
                                      });
...
/*
 * Determines if the passed string is a valid regex string.
 * Returns true if it is, and false if it is not
 */
function isValidRegex(regexString){
  // split on "/" and if the length is greater than equal
  // to 3. Then it is probably a regex. To verify we check
  // if there is an empty string in the first split
  // position and if there is either an empty string in
  // the last position or if there is only valid regex
  // "modifiers", e.g. "g" or "i" for case-insensitive
  splitted = regexString.split("/");
  ...
  if(splitted[splitted.length-1] != ""){
    allowedModifiers = ["g", "i", "m"];
    for(var i = 0; i < splitted[splitted.length].length; i++){
      var inModifiers = false;
      for(var j = 0; j < allowedModifiers.length; j++){
        if(splitted[splitted.length][i] == allowedModifiers){
          inModifiers = true;
        }
      }
      if(!inModifiers){
...
/*
 * Assumes filterText is a regex string that describes what is to
 * be replaced and replaceText is what should be replacing it.
 */
function regexReplace(editor, originalText, filterText, replaceText){
  var newText = "";
  // replace the matched strings with replaceText
  var splitted = filterText.split("/");
  var allRegex = splitted.splice(1, splitted.length-1).join("");
  // splitted[...].includes("i") checks if the i modifier is set
  var modifiers = "gm" + (splitted[splitted.length-1].includes("i") ? "i" : "");
  var regexpression = new RegExp(allRegex, modifiers);

  // console.log("regexp: " + regexpression);
  var lastStartI = 0;
  while ((match = regexpression.exec(originalText)) != null) {
      // console.log("match: " + match + " found at: " + match.index);
      var replacingText = replaceText.replace("\\reg", match);
      newText = newText + originalText.substring(lastStartI, match.index) + replacingText;
      lastStartI = match.index + match.length;
  }

  // add what is left in the buffer
  if(lastStartI != originalText.length){
    newText = newText + originalText.substring(lastStartI, originalText.length);
  }

  // console.log("newText: ", newText);

  editor.insertText(newText);
  // editor.insertText(originalText.replace(filterText, replaceText));
}
...
/*
 * The event handler for when a user presses a key inside of the filter text box
 */
function pressedKeyFilterEHandler(event, _this) {
  var editor = atom.workspace.getActiveTextEditor();
  ...
  var count = "0";
  if(isValidRegex(filterText)){
    count = "" + matchCountRegex(editor, originalText, filterText, replaceText);
  } else {
    count = matchCountNormal(editor, originalText, filterText, replaceText);
  }

  var messageHolder = document.getElementById("matchcount");
  messageHolder.textContent = count;
}

/*
 * Returns the number of matches for the current filterText on the original text with
 * regex.
 */
function matchCountRegex(editor, originalText, filterText, replaceText){
  var splitted = filterText.split("/");
  var allRegex = splitted.splice(1, splitted.length-1).join("");
  // splitted[...].includes("i") checks if the i modifier is set
  var modifiers = "gm" + (splitted[splitted.length-1].includes("i") ? "i" : "");
 ...
 * the non-regex mode.
 */
function matchCountNormal(editor, originalText, filterText, replaceText){
  var regex1 = new RegExp(filterText, "g");
  var matches = originalText.match(regex1);

  return (matches === null || filterText === "" ? 0 : matches.length);
}
